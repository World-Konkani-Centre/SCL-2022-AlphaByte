...
def report(request):
    ...
    all_wastesP = all_wastes.filter(dropdown_done = False).order_by('-pickup_date')
    W3 = Paginator(all_wastesP,10)
    Pwastes = W3.get_page(request.GET.get('page3'))
    all_wastesD = all_wastes.filter(pickup_done = True).order_by('-dropdown_date')
...
def manager(request):
    groups = grouplist(request.user)
    all_wastes_pickup = Waste.objects.filter(pickup_done=False).order_by('-pickup_date')
    ...
    all_wastes_dropdown = Waste.objects.filter(pickup_done=True).order_by('-dropdown_date')
